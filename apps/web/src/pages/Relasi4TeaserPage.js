...
const Relasi4TeaserPage = () => {
  ...
  // --- FIXED: FORCE PRODUCTION MIDTRANS SCRIPT ---
  useEffect(() => {
    // 1. Fetch Teaser Data
    fetchTeaser();
    ...
    // 3. Load Midtrans Script (HARDCODED PRODUCTION)
    const loadSnapScript = async () => {
      try {
        // Ambil client key dari backend
        const response = await axios.get(`${API}/payment/client-key`);
        const { client_key } = response.data; // Kita tidak butuh is_production lagi karena kita memaksa Production
        setMidtransClientKey(client_key);

        // Cek script agar tidak double
        if (document.getElementById('midtrans-snap-script')) {
          setSnapLoaded(true);
          return;
        }

        // --- INI PERUBAHANNYA: LANGSUNG KE URL PRODUCTION ---
        const script = document.createElement('script');
        script.id = 'midtrans-snap-script';
        script.src = 'https://app.midtrans.com/snap/snap.js'; // <--- FORCE PRODUCTION URL
        ...
        script.onload = () => {
            setSnapLoaded(true);
            console.log("Midtrans Production Script Loaded");
        };
  ...
  const handlePurchase = async () => {
    ...
    if (!snapLoaded) {
      toast.error(t("Sistem pembayaran sedang dimuat...", "Payment system loading..."));
      return;
    }
