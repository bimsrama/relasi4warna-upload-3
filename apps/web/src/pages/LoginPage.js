...
const LoginPage = () => {
  const { t } = useLanguage();
  const { login, setToken, setUser } = useAuth(); // Pastikan setToken/setUser terekspos dari useAuth, atau sesuaikan
  ...
  // --- 1. LOGIKA HANDLE BALIKAN DARI GOOGLE ---
  useEffect(() => {
  ...
  const handleGoogleCallback = async (code) => {
    ...
    try {
      // Hapus query param agar bersih
      window.history.replaceState({}, document.title, "/login");

      // Kirim code ke backend
      const res = await axios.post(`${API}/auth/google`, {
        code: code,
        // Harus SAMA PERSIS dengan yang di .env dan Google Console
        redirect_uri: window.location.origin + "/login" 
      });

      // Simpan sesi login (sesuaikan dengan logic useAuth Anda)
      ...
      toast.error(t("Gagal login dengan Google", "Google login failed"));
    } finally {
  ...
  const handleGoogleLogin = () => {
    ...
    const REDIRECT_URI = window.location.origin + "/login"; // Kembali ke halaman ini
    const SCOPE = "openid email profile";
    
    // Redirect browser ke Google Auth URL
